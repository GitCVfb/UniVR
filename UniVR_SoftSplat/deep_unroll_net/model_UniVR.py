...
class ModelUniVR(ModelBase):
    ...
    def forward(self, time, gamma):# -gamma/2 <= time <= 1 - gamma/2, 0 <= gamma <= s
        ...
        gs_t_final = self.net_vfi(self.im_rs, flow_bidir, time, gamma)
    ...
    def optimize_parameters(self):
        ...
        for tt in range(len(timesteps)):
            ...
            pred_im = self.forward(timesteps[tt], self.opts.gamma)
            #===========================================================#
            #                       Compute losses                      #
            #===========================================================#
            gs_gt = im_gt.clone()
        ...
        # sum them up
        self.loss_G = self.loss_L1 + self.loss_perceptual
    ...
    def load_pretrained_GS_model(self, label, save_dir):
        ...
        self.net_vfi.load_state_dict(convert(weights), False)
