...
def interflow_syn(bidir_optical_flow, t, gamma=1.0):# -gamma/2 <= tau <= 1 - gamma/2
    _, _, H, W = bidir_optical_flow.size()#flow: [B, 4, H, W]
...
class SoftSplatBaseline(nn.Module):
    ...
    def forward(self, x, flow, target_t, gamma=1.0):# 0 <= target_t <= 1,  -gamma/2 <= t <= 1 - gamma/2
        #flow: [2B, 2, H, W]
        ...
        for lv in range(n_lv):
            ...
            flow0t, flow1t = interflow_syn(torch.cat((flow01, flow10), 1), target_t, gamma)
