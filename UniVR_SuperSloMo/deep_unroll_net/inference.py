...
class Inference(Generic_train_test):
    ...
    def test(self):
        ...
        #data_save = open("/home1/fanbin/fan/DeepUndisFlow2/DeepVFI_rssr/deep_unroll_results/output.txt",'w+')###save vertical flow values
        ...
        for i, data in enumerate(self.dataloader):
            ...
            if self.opts.load_1st_GS==1:
                with torch.no_grad():
                    pred_im = self.model.forward(1.0-self.opts.gamma, self.opts.gamma)#corresponding to the first scanline of the second RS frame
            else:
                with torch.no_grad():
                    pred_im = self.model.forward(1.0-self.opts.gamma/2.0, self.opts.gamma)#corresponding to the middle scanline of the second RS frame
            torch.cuda.synchronize()
            time_end=time.time()
